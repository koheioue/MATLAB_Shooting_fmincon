...
%% スケーリングファクターを適用
g0               = g0_km_s2 * tsf^2 / lsf; % 重力定数 (km/s^2)
R_sun_earth      = R_sun_earth_km / lsf;   % 太陽と地球の平均距離 (km)
R_earth          = R_earth_km / lsf;       % 地球の半径（km）
R_sun            = R_sun_km / lsf;         % 太陽の半径（km）
R_mars           = R_mars_km / lsf;        % 火星の半径（km）

%% 定数の計算（lsf, tsfでスケーリング）
mu_earth         = 398600.4418 / (lsf^3 / tsf^2); % 地球の重力定数 (km^3/s^2)
mu_sun           = 1.327124e+11 / (lsf^3 / tsf^2); % 太陽の重力定数 (km^3/s^2)
